...
def classify(image):
    ...
    pred = np.argmax(
        pred_probs
    )  # Get the class label with highest probability using np.argmax
...
def detectSign(file):
    ...
    croppedSign = localization(
        frame, min_size_components, similitary_contour_with_circle
    )
...
image_list = []
for filename in glob.glob("D:\\Github\\VisionProject\\Pictures\\Train\\3\\*.jpg"): #assuming gif
...
for i in image_list:
    croppedSign = localization(
        i, min_size_components, similitary_contour_with_circle
    )
